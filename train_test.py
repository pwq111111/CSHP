...
def objective(trial):
    ...
    params = {
        ...
        'attention_num': trial.suggest_int('attention_num',8,25),
        'prompt_length': trial.suggest_int('prompt_length', 1, 5)
    }

    logger = get_logger(logger_name=args.name, log_dir='./result/')
    logger.info('\n' +'epoch:' + str(args.epoch) +'  seed:' + str(args.seed) +'  learning_rate:' + str(params['learning_rate'])+ '  attention_num:' +str(params['attention_num']) +
                '  prompt_position:'+ str(args.prompt_position) + '  prompt_length:' + str(params['prompt_length']) + '  hard_prompt_front:' +
    ...
    prompt_coop = 'a '*params['prompt_length']
    hard_prompt_front = prompt_coop + args.hard_prompt_front
    train_start_time=time.time()
    # torch.manual_seed(args.seed_cpu)
    # torch.cuda.manual_seed(args.seed_gpu)
    model = ppt_model.model_prompt(args.batch_size,params['prompt_length'], args.embedding_length, args.prompt_position, args.prompt_cls_length,
                                    args.prompt_cls_position, args.token_length, prompt_coop,params['attention_num']).cuda()
    ...
    return 100*(right_number0.item()*random_num + right_number1.item()*random_num1) /(random_num1+random_num)
...
# def main():
#
#     logger = get_logger(logger_name=args.name, log_dir='C:/Users/86178/Desktop/论文/ablation/')
...
#                 args.hard_prompt_front + '  hard_prompt_back:' +args.hard_prompt_back)
#     data_start_time = time.time()
#     class_name, train_loader, test_loader, test_loader1, random_num, random_num1 = args.data['data'](args)
#
...
#     test_start_time = time.time()
#     model.eval()
#     right_number0 = ppt_model.test(model, test_loader, class_name[args.class_min:args.class_max], args.class_min, hard_prompt_front, args.hard_prompt_back,
#                                    args.class_num)
#     right_number1 = ppt_model.test(model, test_loader1, class_name[args.class_min1:args.class_max1], args.class_min1, hard_prompt_front, args.hard_prompt_back,
#                                    args.class_num1)
#     # print_table.add_row([time.time() - t1, args.epoch, args.seed, args.attention_num,args.learning_rate,args.prompt_position, args.prompt_length, args.hard_prompt_front,
#     #                      args.hard_prompt_back, 0, right_number0, right_number1,(right_number0*random_num + right_number1*random_num1) /(random_num1+random_num)])
#     logger.info('data cost=' + '{:.2f}'.format(train_start_time-data_start_time) +'  train cost=' +'{:.2f}'.format(test_start_time-train_start_time)
